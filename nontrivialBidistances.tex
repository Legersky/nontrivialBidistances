...
Set $\delta(u_i v_i)=1$ for all $i\in \left\{1, \dots, s\right\}$ and $\delta(e)=0$ otherwise. Let $C$ be a cycle in $G$. If $C\subset M$ or  $C\subset N$, then all labels are 0. Otherwise, $C$ contains some $u_iv_i$ and $u_jv_j$ such that $u_iv_i\neq u_jv_j$. Hence, the label 1 is in $C$ at least twice. If $u_i\neq u_j$ and $v_j\neq v_j$, then there must be at least one edge in $M\cap C$ and at least one edge in $N\cap C$. \komentar{this is not true for general graph and probably neither for Laman graph!!! see variant below}  If $u_i=u_j$, then $v_i$ and $v_j$ are non-adjacent, otherwise $u_iv_i \sim_{\!\!\bigtriangleup} u_jv_j$. Thus, there are at least two edges in $C\cap N$. The case $v_i=v_j$ is analogous. Therefore, the label 0 is in $C$ at least twice.
...
\begin{thm}
\label{thm:connectingEdgesCut}
Let $G=(V,E)$ be a connected graph, $|E|\geq 2$. If there is a set $E_c$ of connecting edges of $G$ such that $E_c$ separates $G$ and the subgraph of $G$ induced by $E_c$ contains no path of length four, then $G$ is rg-colourable.
\end{thm}
\begin{proof}
Let $E'_c$ be a minimal subset of $E_c$ which separates $G$. The idea is following: since the graph $G'=(V, E\setminus E'_c)$ is disconnected, every cycle in $G$ which does not stay in a connected component of $G'$ must contain at least two edges from $E'_c$. Set
$$
\delta(e)=\begin{cases} \text{red} & \text{if } e\in E'_c \\
		\text{green} & \text{otherwise,} \end{cases}
$$
for all $uv \in E$. The existence of an almost g-cycle with a red edge $u_rv_r$ contradicts minimality of $E'_c$: since $u_r$ and $v_r$ are connected by an g-path, they belong to the same connected component in $G'$. Hence $E'_c\setminus\{u_ru_v\}$ separates $G$. 

On the other hand, assume that there exists an almost r-cycle $C$. Since there is no path of length four in the subgraph of $G$ induced by $E_c$, the length of $C$ is three or four. The length three contradicts the fact the red edges are in $E_c$, i.e., they are connecting edges by the assumption. If $C$ has length four and the red edges are $ u_1u_2, u_2u_3, u_3u_4$, then $G'$ has at least three connected components, since there are no almost g-cycles, i.e., $u_1$ and $u_4$ are in the different component then $u_2$ and $u_3$, and $u_2$ and $u_3$ are not in the same component. But the minimality of $E'_c$ implies that $G'$ has only two connected component which is a contradiction.

The surjectivity of $\delta$ follows from minimality of $E_c$ and the assumption $|E|\geq 2$.
\end{proof}
...
Each multiple vertex with multiplicity $k$ can connect at most $k-1$ \trcomps{} to the previous one. Thus, at most $1+\sum_{k=2}^n \cv{k}(k-1)$ \trcomps{} can be connected by multiple vertices. Therefore, if $1+\sum_{k=2}^n \cv{k}(k-1) <n$, then there exists a \trcomp{} connected only by connecting edges. Theorem~\ref{thm:cutToTwoParts} implies that $G$ has a nontrivial bidistance. \komentar{potrebuje okomentovat varianty kdyz connecting edges tvor path delsi nez tri !!!}
\end{proof}
...
\begin{figure}[htb!]
\centering
\input{img/IIbnotConnectingVertex.tex}
\caption{Bidistance}

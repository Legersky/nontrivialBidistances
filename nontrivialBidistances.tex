...
\begin{lem}[Josef]
\label{lem:noInfFibreManifolds}
Let $X$ and $Y$ be differentiable manifolds. If $f:X\rightarrow Y$  is a differentiable map such that each fibre is discrete, then $\dim X \leq \dim Y$.
\end{lem}
\begin{proof}
Let $p \in X$ be a point where the rank $r$ of the Jacobian $J_f(p)$ is maximal. Let $S:=\left\{x \in X \colon \rank J_f(x)<r \right\}$. A point $x$ is in $S$ iff all minors $r\times r$ of $J_f(x)$ vanish. The minors are polynomial functions of the entries of $J_f(x)$. Since $f$ is differentiable, the set $S$ is the fibre of 0 under continuous mapping, i.e., it is closed. Hence, there exists  an open neighbourhood $U$ of $p$ where the Jacobian $J_f$ has the constant rank $r$. By the constant rank theorem for manifolds (Boothby, Chapter III, Remark 4.2), for every point of U - in particular for p itself - there is an open
subset V such that after some differentiable change of coordinates, the map f
restricted to V becomes linear. If a linear map has discrete fibres, then the
dimension of the domain is smaller than or equal to the dimension of the codomain.
\end{proof}

\begin{lem}
\label{lem:noInfFibre}
Let $X$ and $Y$ be algebraic varieties, $Y$ nonsingular. If $f:X\rightarrow Y$  is a polynomial map  and $\dim X > \dim Y$, then there exists $y\in Y$ such that the fibre $f^{-1}(y)$ is infinite.
\end{lem}
\begin{proof}
Let $X'$
\end{proof}
...
\begin{thm}[Josef]
Let $G$ be a Laman graph with a subgraph $H$ which is also Laman. If $H$ has a nonrigid labeling $\lambda_Hs$, then there exists a nonrigid labeling $\lambda_G$ of $G$.
\end{thm}
\begin{proof}
Fix an edge $\bar{u}\bar{v}\in V_H$. Let $$\mathcal{E}_G=\left\{((x_u,y_u))_{|V_G|}\in (\RR\times\RR)^{|V_G|} \colon x_{\bar{u}}=y_{\bar{u}}=y_{\bar{v}}=0\right\}$$ be the set of all embeddings of $G$ into $\RR^2$ which have the fixed position of the vertex $\bar{u}$ and orientation of the edge $\bar{u}\bar{v}$.
We define a map $f_G:\mathcal{E}_G \rightarrow (\RR^+)^{|E_G|}$ by 
$$
f_G(\varepsilon)=((x_u-x_v)^2+(y_u-y_v)^2)_{uv\in E_G}\,.
$$
...
Now, the fibre $$A:=f_H^{-1}(\lambda_H)$$ is an algebraic variety of a positive dimension, since by assumption there are infinitely many embeddings compatible with $\lambda_H$. 

Let $$B:=\left\{\varepsilon\in \mathcal{E}_G \colon f_H(\varepsilon|_{V_H})=\lambda_H\right\}$$ be all embeddings of $G$ whose restriction to $H$ is compatible with $\lambda_H$. The dimension of $B$ is $\dim A+2|V_G|-2|V_H|$.

Set $$C:=\left\{\lambda \in (\RR^+)^{|E_G|} \colon \lambda|_{E_H}=\lambda_H\right\}\,.$$ Its dimension is $|E_G|-|E_H|$.

By assumption that $H$ is Laman, we have
$$
\dim C=|E_G|-|E_H|=2(|V_G|-|V_H|)<2(|V_G|-|V_H|)+\dim A=\dim B\,.
$$
Lemma \ref{lem:noInfFibre} applied on the map $f_G|_B:B\rightarrow C$ implies that there exists a labeling $\lambda_G$ of $G$ which has an infinite fibre $f_G^{-1}(\lambda_G)$, i.e., it is nonrigid labeling of $G$.
...
We fix a point $p\in X$. Since $X$ is open, we can apply constant rank theorem (Boothby, Chapter II, Theorem 7.1): there exists open subsets $A\subset X$ and $B\subset f(X)$ with $p\in A$ and $f(p)=0\in B$, open subsets $U\subset \RR^n$ and $W\subset \RR^m$ and diffeomorphisms $\varphi:A\rightarrow U$ and $\psi:B\rightarrow W$such that $\psi \circ f \circ \varphi^{-1}(U)\subset W$ and 
$$
\psi \circ f \circ \varphi^{-1}(\tilde{x}_1,\dots, \tilde{x}_n)=(\tilde{x}_1,\dots, \tilde{x}_r, 0, \dots, 0)\,,
$$
where $r=\rank J_p(f)$.

Let $H\subset A$ be a neighbourhood  of $p$. Now
\begin{align*}
 H &= f^{-1}(0) \cap H \\
 &= \left\{\varphi^{-1}(\tilde{x}_1,\dots, \tilde{x}_n)=(x_1,\dots, x_n)\in H \colon f(x_1,\dots, x_n)=0\right\} \\
 &= \varphi^{-1} \left\{(\tilde{x}_1,\dots, \tilde{x}_n)\in \varphi(H) \colon f\circ \varphi^{-1}(\tilde{x}_1,\dots, \tilde{x}_n)=0\right\} \\
 &= \varphi^{-1} \left\{(\tilde{x}_1,\dots, \tilde{x}_n)\in \varphi(H) \colon \psi \circ f\circ \varphi^{-1}(\tilde{x}_1,\dots, \tilde{x}_n)=\psi(0)\right\} \\
 &= \varphi^{-1} \left\{(\tilde{x}_1,\dots, \tilde{x}_n)\in \varphi(H) \colon (\tilde{x}_1,\dots, \tilde{x}_r, 0, \dots, 0)=\psi(0)\right\} \\
 &= \varphi^{-1} \left\{(\tilde{x}_1,\dots, \tilde{x}_n)\in \varphi(H) \colon (\tilde{x}_1,\dots, \tilde{x}_r)=(\psi_1(0),\dots , \psi_r(0))\right\}\,.
\end{align*}
Since $\varphi$ is a homeomorphism, the set $S=\left\{(\tilde{x}_1,\dots, \tilde{x}_n)\in \varphi(H) \colon (\tilde{x}_1,\dots, \tilde{x}_r)=(\psi_1(0),\dots , \psi_r(0))\right\}$ is open.
Let  $\pi:\RR^n\rightarrow \RR^{n-r}$ be the projection given by $\pi(\tilde{x}_{1},\dots, \tilde{x}_n)=(\tilde{x}_{r+1},\dots, \tilde{x}_n)$. The mapping $\pi'$ given by restriction of $\pi$ to $(\psi_1(0),\dots , \psi_r(0)) \times \RR^{n-r}$ is diffeomorphism. We can conclude that
\begin{align*}
 H= \varphi^{-1} \pi'^{-1} S' \,
\end{align*}
where $S'=\pi'(S)$ is an open subset of $\RR^{n-r}$. Since $d=n-r$, $X$ is $d$-dimensional manifold.
\end{proof}

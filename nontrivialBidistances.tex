...
\begin{enumerate}[i)]
	\item There are \trcomps{} $T_1$ and $T_2$ linked by distinct connecting edges $e_1, e_2$ and $e_3$.
	\item There are \trcomps{} $T_1$ and $T_2$ linked by a multiple vertex $v$ and connecting edge $e$.
	\item There are two connecting edges $e_1=uv_1$ and $e_2=uv_2$ with the same vertex $u$ such that $v_1$ and $v_2$ are in the same \trcomp{} $T$.
	\item There are three \trcomps{} such that each two of them are linked by a multiple vertex.
\end{enumerate}
\end{cor}
\begin{proof}

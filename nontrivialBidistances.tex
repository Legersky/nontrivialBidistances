...
\begin{lem}
Let $T=(V_T,E_T)$ be a \trcomp{} of a graph $G$ with connecting vertices $v_1, \dots, v_r$ and connecting edges $u_1u'_1,  \dots ,u_su'_s$ where $u_1, \dots u_s \in V_T$ and $u'_1, \dots u'_s \in V_T$. If $L=(V_L,E_L)$ is a Laman graph with vertices $V_L=\{v_1, \dots, v_r\}\cup\{u_1, \dots u_s\}$, then the graph $G'=((V\setminus V_T)\cup V_L, (E\setminus V_T)\cup E_L$, i.e., the \trcomp{} $T$ is replaced by $L$, is a Laman graph iff $G$ is a Laman graph.
\end{lem}

...
\section{new claims}
\begin{thm}
Let $G$ be a Laman graph and let $H$ be its subgraph. If $H$ is Laman and has a nonrigid labeling, then $G$ has nonrigid labeling.
\end{thm}
\begin{proof}
Let $(\lambda_e)_{e\in E_H}$ be a nonrigid labeling of $H$. Fix an edge $\bar{u}\bar{v}\in E_H$. Let $(x_u, y_u)$ be coordinates of vertex $u\in V_H$. Let $X_H\subset \RR^{|V_H|}\times \RR^{|V_H|}$ be the set of solutions   of the following system of equation:
\begin{align*}

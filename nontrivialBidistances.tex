...
\end{enumerate}
A graph $G$ is called \emph{problematic}, if it is basic problematic or it was derived from a problematic graph $G'$ by Henneberg move Ia, IIc (such that the removed edge is only in one triangle).
\end{defn}
...
$$
Let $v$ be constructed so that it is linked by edges to the vertices $u,u_1,u_2 \in V'$ such that $uu_1\in E',uu_2\notin E'$ and the edge $u_1u_2\in E'$ is removed. There are several cases:
\begin{enumerate}
\item If $\deg u=\deg u_1=\deg v=3$, then there is a nonrigid labelling and nontrivial bidistance by Theorem~\ref{thm:cutToTwoParts}.
\item Let $\deg u=\deg v=3, \deg u_1\geq 4$ and $wu_2\notin E$, where $N_G(u)=\{v,u_1,w\}$. The vertices $w$ and $u_1$ are nonadjacent, otherwise $u$ can be constructed by IIc. Hence, a nonrigid labelling and nontrivial bidistance for $G$ are obtained by application of Theorem~\ref{thm:cutByVertices} with $V_c=\{u_1,u_2,w\}$.
\item Let $\deg u=\deg v=3, \deg u_1\geq 4$ and $wu_2\in E$, where $N_G(u)=\{v,u_1,w\}$. Again, the vertices $w$ and $u_1$ are nonadjacent, otherwise $u$ can be constructed by IIc.
\end{enumerate}
\end{proof}

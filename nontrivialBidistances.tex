...
\end{thm}
\begin{proof}
We proceed by induction with respect to $|V_T|$. The statement is obvious for triangle. Since $T$ is a Laman graph by Lemma~\ref{lem:noEdgeInSameComponent}, there exists a vertex $v\in V_T$ such that $T$ is constructed from $T'$ by some Henneberg move. By Lemma~\ref{lem:trcompsCannotBeLinked}, $T'$ is a \trcomp{} and $v$ is constructed by Ia or IIc. Hence, $T'$ can be constructed only by Ia by the induction assumption and if $v$ is also constructed by Ia, the claim holds. 

Let $v$ be constructed by linking to $u_1, u_2, u\in V_{T'}$ and removing $u_1u_2$. Lemma~\ref{lem:startWithArbitraryEdge} provides a construction of $T'$ by only Ia which starts with $u_1u_2$. Obviously, we can sort all moves which construct a vertex from $u_1u_2$ so that $u$ is connected as the last one. 
\end{proof}

...
\begin{lem}
Let $L$ and $L'$ are Laman graphs such that $L$ can be replaced by $L'$ in a graph $G$. If $G$ is a Laman graph, then $G_{L\rightarrow L'}$ is also a Laman graph.
\end{lem}
...
\end{thm}
\begin{proof}
We remark that there might be no \trcomps{} ($r=0$) or no connecting edges ($s=r$), but always $s\geq 3$ by Lemma XXX and YYY.

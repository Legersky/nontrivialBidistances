...
If a Laman graph $G$ is constructed by Henneberg moves, then the number of \trcomps{} does not decrease during the construction.
\end{lem}
\begin{proof}
We show that all types of Henneberg moves keep or increase the number of \trcomps{}:
\begin{itemize}
	\item Ia: if the new vertex is connected to an \trcomp{}, then 
\end{itemize}
\end{proof}
...
\end{thm}
\begin{proof}
We proceed by induction with respect to $|V_T|$. The statement is obvious for triangle. Since $T$ is a Laman graph by Lemma~\ref{lem:noEdgeInSameComponent}, there exists a vertex $v\in V_T$ such that $T$ is constructed from $T'$ by some Henneberg move.
\end{proof}

...
Let $S_1, \dots, S_n$ be equivalence classes of the transitive closure of $\sim_{\!\!\bigtriangleup}$. The subgraph of $G$ spanned by $S_i$  is called a \emph{\trcomp{} (triangle-component)} if $|S_i|\geq 3$, and a \emph{connecting edge} otherwise.
\end{defn}
...
\begin{enumerate}[i)]
	\item $\cv{n} \leq 1$.
	\item If $\cv{n} = 1$, then $\cv{k}=0$ for all $2<k<n$.
	\item If $n\geq 5$, then $\cv{n-1} \leq 1$.
	\item If $n\geq 5$ and $\cv{n-1} = 1$, then $\cv{k}=0$ for all $2<k<n-1$.
\end{enumerate}s
\end{col}
\begin{proof}

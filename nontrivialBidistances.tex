...
\begin{lem}
Let $L$ and $L'$ be Laman graphs such that $L$ can be replaced by $L'$ in a graph $G$. If $G$ is a Laman graph, then $G_{L\rightarrow L'}$ is also a Laman graph.
\end{lem}
\begin{proof}
CHECK ALL

Suppose in contradiction that there exist a subgraph $H=(V_H,E_H)$ of $G$ such that $E_H|> 2|V_H|-3$. Then TADY POKRACOVAT!!!!!!!!!!!!!!1
\end{proof}

\begin{cor}
\label{cor:replaceTrcompByLaman}
Let $T=(V_T,E_T)$ be a \trcomp{} of a graph $G=(V,E)$ with connecting vertices $v_1, \dots, v_r$ and connecting edges $u_1u'_1,  \dots ,u_su'_s$ where $u_1, \dots u_s \in V_T$ and $u'_1, \dots u'_s \in V_T$. If $L=(V_L,E_L)$ is a Laman graph with vertices $V_L=\{v_1, \dots, v_r\}\cup\{u_1, \dots u_s\}$ and $T$ is a Laman graph, then the graph $G_{T\rightarrow L}$, i.e., the \trcomp{} $T$ is replaced by $L$, is a Laman graph iff $G$ is a Laman graph.
\end{cor}

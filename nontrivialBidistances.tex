...
\item $d_G(e)+d_H(\rho(e))=w_e$ for all $e\in E_G\setminus \{\bar{e}\}$,
...
%\begin{align}
%\label{eq:forH}
%(x_u-x_v)^2 + (y_u-y_v)^2 &= \lambda_{uv} \quad \text{ for all } uv\in E_H \setminus \{\bar{u}\bar{v}\} \,,\nonumber\\
...
\end{lem}
\begin{proof}
Let $p \in X$ be a point where the rank $r$ of the Jacobian $J_f(p)$ is maximal. Let $S:=\{x \in X \colon \rank J_f(x)<r \}$. A point $x$ is in $S$ iff all minors $r\times r$ of $J_f(x)$ vanish. The minors are polynomial functions of the entries of $J_f(x)$. Since $f$ is differentiable, the set $S$ is the fibre of 0 under continuous mapping, i.e., it is closed. Hence, there exists  an open neighbourhood $U$ of $p$ where the Jacobian $J_f$ has the constant rank $r$. By the
"constant rank theorem", for every point of U - in particular for p itself - there is an open
...
Let $X$ and $Y$ be algebraic varieties, $Y$ nonsingular. If $f:X\rightarrow Y$  is a polynomial map  and $\dim X \leq \dim Y$, then there exists $y\in Y$ such that the fibre $f^{-1}(y)$ is infinite.
\end{lem}
\begin{proof}
Let $X'$
\end{proof}
...
\end{thm}
\begin{proof}
Fix an edge $\bar{u}\bar{v}\in V_H$. Let $$\mathcal{E}_G=\{((x_u,y_u))_{|V_G|}\in (\RR\times\RR)^{|V_G|} \colon x_{\bar{u}}=y_{\bar{u}}=y_{\bar{v}}=0\}$$ be the set of all embeddings of $G$ into $\RR^2$ which have the fixed position of the vertex $\bar{u}$ and orientation of the edge $\bar{u}\bar{v}$.
...
Now, the fibre $$A:=f_H^{-1}(\lambda_H)$$ is an algebraic variety with a positive dimension, since by assumption there are infinitely many embeddings compatible with $\lambda_H$. 

Let $$B:=\{\varepsilon\in \mathcal{E}_G \colon f_H(\varepsilon|_{V_H})=\lambda_H\}$$ be all embeddings of $G$ whose restriction to $H$ is compatible with $\lambda_H$. The dimension of $B$ is $\dim A+2|V_G|-2|V_H|$.

Set $$C:=\{\lambda \in (\RR^+)^{|E_G|} \colon \lambda|_{E_H}=\lambda_H\}\,.$$ Its dimension is $|E_G|-|E_H|$.

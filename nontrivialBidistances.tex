...
"constant rank theorem" (this is in most differential geometry books, even in
wikipedia), for every point of U - in particular for p itself - there is an open
subset V such that after some differentiable change of coordinates, the map f
restricted to V becomes linear. If a linear map has discrete fibers, then the
...
Now, the fibre $$A:=f_H^{-1}(\lambda_H)$$ has a positive dimension, since by assumption there are infinitely many embeddings compatible with $\lambda_H$.

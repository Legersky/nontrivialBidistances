...
Let  $\pi:\RR^n\rightarrow \RR^{n-r}$ be the projection given by $\pi(\tilde{x}_{1},\dots, \tilde{x}_n)=(\tilde{x}_{r+1},\dots, \tilde{x}_n)$. The mapping $\pi'=\pi|_Z$ is a diffeomorphism from $Z$ onto $\RR^{n-r}$. Hence, $\pi'\circ \varphi (H)= \pi'( U \cap Z )$ is open in $\RR^{n-r}$, i.e., $(H,\pi'\circ\varphi|_H$ is a chart of $X$. Since $d=n-r$, $X$ is $d$-dimensional manifold.
\end{proof}

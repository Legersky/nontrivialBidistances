...
REFERENCE TO FOLLOWING LEMMA IS MISSING:
\begin{lem}
\label{lem:noInfFibre}
Let $X$ and $Y$ be algebraic varieties and $f:X\rightarrow Y$ be morphism. If $f$ has no infinite fibre, then $\dim X \leq \dim Y$.
\end{lem}
...
$$
Lemma \ref{lem:noInfFibre} applied on the map $f_G|_B:B\rightarrow C$ implies that there exists a labeling $\lambda_G$ of $G$ which has an infinite fibre $f_G^{-1}(\lambda_G)$, i.e., it is norigid labeling of $G$.

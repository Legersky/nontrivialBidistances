...
If there is a triangle induced by vertices $u,v, u_1\in V$ such that $\deg_G u =\deg_G v=3$ and $\deg_G u_1>3$, then there are two possibilities. Let $w\in V\setminus \{v, u_1\}$ be the neighbour of $u$ and $u_2\in V\setminus \{u, u_1\}$ be the neighbour of $v$. The easiest case is if $wu_2\notin E$: the vertices $w$ and $u_1$ are nonadjacent, otherwise $u$ can be constructed by IIc. Hence, a nonrigid labelling and nontrivial bidistance for $G$ are obtained by application of Theorem~\ref{thm:cutByVertices} with $V_c=\{u_1,u_2,w\}$. If $wu_2\notin E$, then Lemma~\ref{lem:undoIIb} justifies that $G'$ can be indeed such that the vertex $v$ is constructed so that it is linked by edges to the vertices $u,u_1,u_2 \in V'$ such that $uu_1\in E',uu_2\notin E'$ and the edge $u_1u_2\in E'$ is removed, or the situation is symmetric by replacing $v$ by $u$ and $w$ by $u_2$. Assume the former one. The degree of $u$ in $G'$ is two. Therefore, $G'$ can be obtain from $G''$ by adding vertex $u$ by Ib. The sum of connecting edges and \trcomps{} in $G$ is at least five: \trcomp{} induced by vertices $u,v, u_1\in V$, connecting edges $uw$ and $u_2v$, \trcomp{} containing $u_2w$ (or it is just a connecting edge)
\end{proof}

...
\begin{cor}
Let $T_1,T_2$ and $T_3$ be distinct \trcomps{} of a Laman graph. If $T_1$ has only two connecting vertices $u$ and $v_1$, $T_2$ has only two connecting vertices $u$ and $v_2$ and $v_1, v_2\in T_3$, then  the graph has a nontrivial bidistance.
\end{cor}
\begin{proof}
We have $uv_1\notin E$ or $uv_2\notin E$ or $v_1v_2\notin E$, otherwise $T_1,T_2$ and $T_3$ coincide.
...
\delta(e)&=1 \qquad \text{if } e\in T_1\cup T_2 \\
		&=0 \qquad \text{otherwise.}
\end{align*} 
If a cycle goes through $T_1$ or $T_2$, then it must go through  $v_1, u$ and $v_2$. Therefore, 1 is there at least twice. At the same time, there are at least two 0 since  $v_1v_2\notin E$.
\end{proof}

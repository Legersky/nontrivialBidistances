...
\begin{itemize}
	...
	\item A graph $G$ has a \emph{nonrigid labelling OR EMBEDDING/LABELLING?} iff there exists a labelling $\lambda$ such that the number of embeddings of $G$ compatible with $\lambda$ is up to rotations and translations is infinite.
\end{itemize} 
\end{defn}
...
\block{Conjecture}{
Let $G$ be a Laman graph. The following statements are equivalent:
\begin{enumerate}[i)]
